    ...
    <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1}} transition={{ delay: 3 }} viewport={{ once: true }}>
      <div className='about-section'>
        ...
        <div className='about-desc'>
          <motion.h3 initial={{ opacity: 0, y: 100 }} whileInView={{ opacity: 1, y: 0, ease: easeInOut }} transition={{ duration: 1.2, delay: 1}} viewport={{ once: true }}>When I’m not busy coding, I also enjoy listening to <span onMouseEnter={() => setMusicHover(true)} onMouseLeave={() => setMusicHover(false)}>music</span>, watching <span onMouseEnter={() => setAnimeHover(true)} onMouseLeave={() => setAnimeHover(false)}>anime</span>, and searching for the next show or movie on <span onMouseEnter={() => setNetflixHover(true)} onMouseLeave={() => setNetflixHover(false)}>Netflix</span> to binge while laying on my bed.</motion.h3>
